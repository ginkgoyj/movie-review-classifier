...
OUTPUT_DIR = BASE_DIR
# Store downloaded NLTK resources next to this script to avoid global setup issues.
...
def ensure_nltk_resource(resource_path, package_name):
    try:
        ...
        # Download the missing corpus/tokenizer only when it is not available locally.
...
def preprocess_text(text):
    ...
    text = text.lower()
    # Remove simple HTML markup that may appear in raw review content.
    text = re.sub(r"<[^>]+>", " ", text)
    # Keep alphabetic tokens only, then remove stop words and apply stemming.
...
def save_model_comparison(nb_scores, svm_scores):
    # Draw a side-by-side bar chart for the main evaluation metrics of both models.
...
def save_confusion_matrices(y_test, nb_preds, svm_preds):
    # Save confusion matrices so the two classifiers can be compared visually.
